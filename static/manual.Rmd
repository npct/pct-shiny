...
<td style="vertical-align: top; width: 535px;">![Scenario Selection](../www/assets/help/scenario-selection.png)  
</td>
...
<td style="vertical-align: top; width: 535px;">![Zone Attribute](../www/assets/help/zone-attribute.png)  
</td>
...
<td style="vertical-align: top; width: 535px;">![Cycling Flows](../www/assets/help/cycling-flows.png)  
</td>
...
<td style="vertical-align: top; width: 535px;">![Fixed Lines](../www/assets/help/freeze-lines.png)  
</td>
...
<td style="vertical-align: top; width: 535px;">![Line Attribute](../www/assets/help/line-attribute.png)  
</td>
...
<td style="vertical-align: top; width: 535px;">![Number of Lines to Show](../www/assets/help/number-of-lines.png)  
</td>
...
<td style="vertical-align: top; width: 535px;">![Direct Route](../www/assets/help/direct-route.png)  
</td>
...
<td style="vertical-align: top;">![Quiet Route](../www/assets/help/quiet-route.png)  
</td>
...
<td style="vertical-align: top;">![Straight Line](../www/assets/help/straight-line-popup.png)  
</td>
...
<td style="vertical-align: top;">![Zone](../www/assets/help/zone-popup.png)  
</td>
...
<td style="vertical-align: top;">![Legend](../www/assets/help/legend.png)</td>
...
<td align="center" valign="center">[![](../www/assets/FAQs/Fig0.png)](../www/assets/FAQs/Fig0.png "Figure 1: Proportion of trips cycled in England, according to the hilliness of the local area")   
</td>
...
<td align="center" valign="center">[![](../www/assets/FAQs/Fig1.png)](../www/assets/FAQs/Fig1.png "Figure 2: Distribution of the population of the Netherlands in terms of the hilliness of their local area, relative to the tenths defined in England.") </td>
...
<td align="center" valign="center">[![](../www/assets/FAQs/Fig2.png)](../www/assets/FAQs/Fig2.png "Figure 1: Proportion of trips cycled in England and Netherlands stratified by age and sex")   
</td>
...
<td align="center" valign="center">[![](../www/assets/FAQs/Fig3.png)  
](../www/assets/FAQs/Fig3.png "Figure 2: Ratio of cycle mode share in the Netherlands versus England, stratified by age and sex")</td>
...
<td align="center" valign="center">[![](../www/assets/FAQs/Fig4.png)](../www/assets/FAQs/Fig4.png "Figure 3: 'Distance decay' curves in England and the Netherlands, showing the probability that a trip of a given distance will be cycled relative to a trip of 2 miles")   
</td>
...
<td align="center" valign="center">[![](../www/assets/FAQs/Route.png)](../www/assets/FAQs/Route.png "Direct and Quiet Routes between two zones.")   
</td>
...
To create custom scenarios, the key dataset to modify is 'l.Rds', a SpatialLinesDataFrame (an R object class) that exists in each region. In the *root directory* of the RStudio project for the `pct-load` folder (which can be opened by opening [`pct-load.Rproj`](https://github.com/npct/pct-load/blob/master/pct-load.Rproj)), this could be loaded for kent, for example, with the folowing commands, resulting in the follow plot:
...
[8\. Why is there no increase in cycling under Go Dutch?](javascript:toggleElement('a8'))

<div id="a5" style="display:none">

On a very few routes, almost all in Cambridge, there is no increase in cycling under the Go Dutch scenario. This means that between these places cycling is already at Dutch levels. It does not mean that there is no potential for more cycling in those areas. The evidence we use from the Netherlands is based on average values for the whole country. However, even within the high cycling Netherlands there is variation between regions:
...
<td style="vertical-align: top; width: 1054px;"><span style="font-weight: bold; text-decoration: underline;">% trips less than 8 miles that are cycled:</span>  
...
Within these regions there will be further variation between high and very high cycling areas.

For most of England, using an average Go Dutch scenario would represent a massive increase in cycling and represents a suitably ambitious goal. However, Cambridge is an extreme outlier in English cycling. Cambridge has a strong cycling culture, an old dense centre that is hard to pass by car, and a high proportion of students. So for Cambridge a scenario should be more ambitious, and perhaps a ‘Go Groningen’ scenario would be more appropriate. 

</div>

[9\.What does a negative health benefit mean?](javascript:toggleElement('a9'))

<div id="a9" style="display:none">

The health economic impact is composed of two parts. The health gain comes from calculating the reduction in premature deaths due to the increase in physical activity from the new cycling or e-bike trips.

However, we also have to account for any trips switched from walking to cycling. For these trips there is an overall reduction in physical activity. If a trip is switched to e-bikes the reduction in physical activity is larger.

As such, the overall effect on health along any route is calculated by estimating the health gain from the scenario increase in cycling and ebiking, minus the health loss from any decrease in walking.  In the vast majority of areas the overall effect is a decrease in mortality (99.7% of MSOAs show a health gain at the area level).  For some routes, however, the health impact can be negative if a large proportion of the new cycling trips were previously walked. This is particularly likely when applying scenarios to short routes in city centres, where walking often makes up a very high proportion of all commuter trips of baseline.  This is true of only a relatively small proportion of routes: 5.9% in the Government Target scenario, 4.0% in the Gender Equity scenario, 6.8% in the Go Dutch scenario, and 15.9% in the Ebikes scenario.  However, these routes may be particularly visible when using PCT as they will usually have a short distance, and so have high cycling potential.

Overall for England as a whole the health and health economic benefits from the increase in cycling are large and this is true for all regions and town and cities. 

**E-bikes**
The proportion of routes with a negative health impact is greater for e-bikes as e-bikes require less physical activity than regular bikes.  However, e-bikes also enable people to cycle in hillier areas and on longer journeys, few of which are walked at the moment. It is possible that e-bikes would facilitate cycling by people who are currently less active and less likely to use regular bikes but we have not modelled this.

**Does a negative health impact mean we shouldn’t invest in cycling?**
Even when there is a negative health impact for a route between two areas, it may still make sense to invest in cycling. Firstly this route is likely to be close to a city centre and thus a key part of a wider network enabling people to travel into the city by bike from further away. These longer trips will have a positive health benefit as few are currently walked. The cycling potential for a route can best be seen via the route network layer. 

Secondly, when cycling is replacing walking there will be time savings, and these will be even greater for e-bikes. We have not currently modelled travel time changes. For car trips, whether cycling is faster will be context specific as it depends on congestion levels. However, cycling will almost always be faster than walking. It should also be noted that in some cases people will be walking as they do not have access to a car, and thus investing in cycling provides more travel choices for lower income groups.

**Overall**, clearly societal benefits are greater if cycling replaces cars rather than walking and these results are a reminder of the importance of maintaining and improving the walking environment. However, increasing transport options and providing access to a faster form of active transport does also bring benefits. 

</div>

<!-- [10\. How can the PCT be used to build a business case for cycling?](javascript:toggleElement('a10'))
...
# copy figures into assets folder
m = "cp -v regions_www/manual_files/figure-html/* www/assets/"
system(m)
# change image links
f = readLines("regions_www/manual.html")
f = gsub(pattern = '(?<=<img src=")(.*)(?=assets/)',
         replacement = "./kent/", f, perl = T)
f = gsub(pattern = "../www/", replacement = "kent/", x = f) # point to kent for now
f = gsub(pattern = 'manual_files', replacement = "index_files", f, perl = T)
f = gsub(pattern = "index_files/figure-html/", replacement = "./kent/assets/", f)
...
f = gsub(pattern = "../www/", replacement = "", x = f)
f = f[-c(1:sel_head)]
